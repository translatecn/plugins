...
import (
	...
	"github.com/containernetworking/plugins/plugins/ipam/over/host-local/backend/disk"
)
...
var _ = Describe("host-local Operations", func() {
	...
	for _, ver := range over.AllSpecVersions {
		...
		It(fmt.Sprintf("[%s] allocates and releases addresses on specific interface with ADD/DEL", ver), func() {
			...
			// Allocate the IP
			r0, raw, err := over.CmdAddWithArgs(args0, func() error {
				return cmdAdd(args0)
			})
			Expect(err).NotTo(HaveOccurred())
			if over.SpecVersionHasIPVersion(ver) {
			...
			// Allocate the IP
			r1, raw, err := over.CmdAddWithArgs(args1, func() error {
				return cmdAdd(args1)
			})
			Expect(err).NotTo(HaveOccurred())
			if over.SpecVersionHasIPVersion(ver) {
			...
			err = over.CmdDelWithArgs(args0, func() error {
				return cmdDel(args0)
			})
			Expect(err).NotTo(HaveOccurred())
			...
			err = over.CmdDelWithArgs(args1, func() error {
				return cmdDel(args1)
			})
			Expect(err).NotTo(HaveOccurred())
		...
		It(fmt.Sprintf("[%s] repeat allocating addresses on specific interface for same container ID with ADD", ver), func() {
			...
			// Allocate the IP
			r0, raw, err := over.CmdAddWithArgs(args, func() error {
				return cmdAdd(args)
			})
			Expect(err).NotTo(HaveOccurred())
			if over.SpecVersionHasIPVersion(ver) {
		...
		It(fmt.Sprintf("[%s] does not output an error message upon initial subnet creation", ver), func() {
			...
			// Allocate the IP
			_, out, err := over.CmdAddWithArgs(args, func() error {
				return cmdAdd(args)
			})
			Expect(err).NotTo(HaveOccurred())
