...
func (s *Store) LastReservedIP(rangeID string) (net.IP, error) {
	ipfile := GetEscapedPath(s.dataDir, lastIPFilePrefix+rangeID) // /tmp/host-local_test976000211/mynet/last_reserved_ip.0
...
func (s *Store) Reserve(id string, ifname string, ip net.IP, rangeID string) (bool, error) {
	...
	if _, err := f.WriteString(strings.TrimSpace(id) + LineBreak + ifname); err != nil { // /tmp/host-local_test2357479361/mynet/10.1.2.2
		f.Close()
		os.Remove(f.Name())
		return false, err
	}
	...
	ipfile := GetEscapedPath(s.dataDir, lastIPFilePrefix+rangeID) // /tmp/host-local_test2357479361/mynet/last_reserved_ip.0
	err = os.WriteFile(ipfile, []byte(ip.String()), 0o600)        // 10.1.2.2
	if err != nil {
		return false, err
	}
	return true, nil
