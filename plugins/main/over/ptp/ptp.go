...
func setupContainerVeth(netns ns.NetNS, ifName string, mtu int, pr *current.Result) (*current.Interface, *current.Interface, error) {
	...
	// IPAM的结果类似于IP=192.168.3.5/24, GW=192.168.3.1。
	//我们想要的是一个点对点链接，但是不支持iff_pointpoint。
	//下一个最好的事情是让它ARP，但设置接口为192.168.3.5/32，并添加一个路由，如“192.168.3.0/24 via 192.168.3.1 dev $ifName”。
	//不幸的是，这将不起作用，因为GW将在接口的子网之外。
	//我们的解决方案是配置接口为192.168.3.5/24，然后删除自动添加的“192.168.3.0/24 dev $ifName”路由。然后我们添加“192.168.3.1/32 dev $ifName”和“192.168.3.0/24 via 192.168.3.1 dev $ifName”。
	//换句话说，我们强制所有流量通过网关ARP除了GW本身。
	hostInterface := &current.Interface{}
	containerInterface := &current.Interface{}

	err := netns.Do(func(hostNS ns.NetNS) error { // 会进入到这个ns
		hostVeth, contVeth0, err := ip.SetupVeth(ifName, mtu, "", hostNS) // ptp0
		if err != nil {
			return err
		}
		...
		if err = ipam2.ConfigureIface(ifName, pr); err != nil {
			return err
		}
...
func init() {
	originalNS, _ := testutils.NewNS()
	targetNS, _ := testutils.NewNS()

	dataDir, _ := os.MkdirTemp("", "ptp_test")
	defer func() {
		os.RemoveAll(dataDir)
		originalNS.Close()
		testutils.UnmountNS(originalNS)
		targetNS.Close()
	...
	os.Setenv("CNI_NETNS", targetNS.Path())
	os.Setenv("CNI_IFNAME", "p0")
	os.Setenv("CNI_CONTAINERID", "dummy")
	...
	args := &skel.CmdArgs{
		ContainerID: "dummy",
		Netns:       targetNS.Path(), // /var/run/netns/cnitest-47d71bc0-6080-a958-f9f5-ede7e5a6cffa
		IfName:      "ptp0",
		StdinData:   []byte(conf),
	}
	cmdAdd(args)
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	// Convert whatever the IPAM result was into the current Result type
	result, err := current.NewResultFromResult(r) // r: 040.Result      result:100.Result
	if err != nil {
		return err
	}
