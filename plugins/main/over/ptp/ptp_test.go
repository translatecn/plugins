...
var _ = Describe("ptp Operations", func() {
	...
	doTest := func(conf, cniVersion string, numIPs int, expectedDNSConf types.DNS, targetNS ns.NetNS) {
		...
		args := &skel.CmdArgs{
			...
			//				"type": "host-local",
			//				"subnet": "10.1.2.0/24",
			//				"dataDir": "/tmp/ptp_test183600714"
			//			    },
			//			    "dns": {"nameservers":["10.1.2.123"],"domain":"some.domain.test","search":["search.test"],"options":["option1:foo"]}
			//			}
		}
		...
		err = targetNS.Do(func(ns.NetNS) error {
			...
			for _, ipc := range ips {
				...
				// ping -c 1 -W 30 -I 10.1.2.2 10.1.2.1
	...
	for _, ver := range over.AllSpecVersions {
		...
		It(fmt.Sprintf("[%s] does not override IPAM DNS settings if no DNS settings provided", ver), func() {
			...
			resolvConfPath, err := over.TmpResolvConf(ipamDNSConf)
			Expect(err).NotTo(HaveOccurred())
